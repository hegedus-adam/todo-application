...
class TodoControllerTest {
  ...
  void testReadToDo_ShouldCalledFacadeAndReturnTodo_WhenCalledWithValidId() {
    //given
    when(todoFacade.readToDo("1")).thenReturn(new ResponseEntity<>(item1, HttpStatus.OK));
    //when
    ResponseEntity<ToDo> actual = underTest.readToDo("1");
    //then
    ResponseEntity<ToDo> expected = new ResponseEntity<>(item1, HttpStatus.OK);

    assertEquals(expected, actual);
    verify(todoFacade).readToDo("1");
  }
  ...
  void testUpdateToDo_ShouldCalledFacadeAndReturnUpdatedTodo_WhenCalledWithValidIdAndToDo() {
    ...
    when(todoFacade.updateToDo("1", toDo)).thenReturn(new ResponseEntity<>(toDo, HttpStatus.OK));
    //when
    ResponseEntity<ToDo> actual = underTest.updateToDo("1", updated);
    //then
    ResponseEntity<ToDo> expected = new ResponseEntity<>(toDo, HttpStatus.OK);

    assertEquals(expected, actual);
    verify(todoFacade).updateToDo("1", toDo);
  }

  @Test
  void testDeleteToDo_ShouldCalledFacadeAndReturnId_WhenCalledWithValidId() {
    //given
    when(todoFacade.deleteToDo("1")).thenReturn(new ResponseEntity<>("1", HttpStatus.OK));
    //when
    ResponseEntity<String> actual = underTest.deleteToDo("1");
    //then
    ResponseEntity<String> expected = new ResponseEntity<>("1", HttpStatus.OK);

    assertEquals(expected, actual);
    verify(todoFacade).deleteToDo("1");
  }
}
